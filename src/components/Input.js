...
export default function Input({ setList, id, setId }) {
  ...
  const handleClick = () => {
    ...
    if (!title) {
      ...
      setError('');
      setList((list) => [...list, {id, title, isComplete: false}]);
      setId(id+1);
      setTodo({title: ''});
    }
  };
  ...
  return (
    <div className="input">
      <div>
        <div className="form">
          <input type="text" onChange={handleChange} value={todo.title} autoComplete="off" spellCheck="false" required />
          <label className="label-name">
            <span className="content-name">Enter the Title</span>
          </label>
        </div>
